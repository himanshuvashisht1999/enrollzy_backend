...
@section('content')
    <div class="container-fluid">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form id="addProjectForm" action="{{ route('admin.projects.store') }}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <select class="form-control" name="lead_source_id" id="lead_source_id">
                                @foreach ($leadSource as $source)
                                    <option {{ old('lead_source_id') == $source->id ? 'selected' : '' }}
                                        value="{{ $source->id }}">{{ $source->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4 form-group">
                            <label for="client_id">Project User ( *Default Personal )</label>
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <select class="form-control" name="status" required>
                                <option {{ old('status') == 'not_started' ? 'selected' : '' }} value="not_started">Not
                                    Started</option>
                                <option {{ old('status') == 'in_progress' ? 'selected' : '' }} value="in_progress">In
                                    Progress</option>
                                <option {{ old('status') == 'completed' ? 'selected' : '' }} value="completed">Completed
                                </option>
            ...
            <div class="card-footer text-right">
                ...
                <button class="btn btn-primary btn-sm" form="addProjectForm" type="submit">Add Project</button>
            </div>
        </div>
    </div>
@endsection
