    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">Add Milestones</h6>
            </div>
            <div class="card-body">
                <form id="addMilestonesForm" action="{{ route('admin.milestones.store') }}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <select class="form-control" name="project_id" required>
                                @foreach ($project as $prjct)
                                    <option value="{{ $prjct->id }}">{{ $prjct->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4 form-group">
                            ...
                            <select class="form-control" name="status" required>
                                <option value="incomplete" {{ old('status') == 'incomplete' ? 'selected' : '' }}>IN Complete
                                </option>
            ...
            <div class="card-footer text-right">
                ...
                <button class="btn btn-primary btn-sm" form="addMilestonesForm" type="submit">Add Milestones</button>
            </div>
        </div>
    </div>
@endsection
