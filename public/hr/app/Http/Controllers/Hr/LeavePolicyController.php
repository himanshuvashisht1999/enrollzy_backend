...
class LeavePolicyController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
                ...
                if(Auth::guard('admin')->user()->role === 'superadmin'){
                    ...
                    $data = LeavePolicy::where('organization_id', Auth::guard('admin')->user()->organization_id)->orderBy('created_at', 'desc')->get();
                }
            ...
            return DataTables::of($data)->addIndexColumn()
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href="' . route('admin.leavePolicy.edit', encrypt($row->id)) . '" class="btn btn-sm"><i
                            class="fa fa-edit text-success"></i></a>';
                    $btn .= ' | ';
                    $btn .= '<form method="POST" action="' . route('admin.leavePolicy.destroy', encrypt($row->id)) . '" class="m-0 p-0">
        ...
        return view('hr.leavesPolicy.index');
    }
    ...
    public function create()
    {
        ...
        return view('hr.leavesPolicy.create', compact('department', 'designation'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect(route('admin.leavePolicy.index'))->with('success', 'Leave Type created successfully.');
        } catch (Exception $e) {
    ...
    public function edit($id)
    {
        ...
        if(Auth::guard('admin')->user()->role === 'superadmin'){
            ...
            ->get();
            return view('hr.leavesPolicy.edit', compact('lSetting', 'department', 'designation','users'));
        } catch (Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect(route('admin.leavePolicy.index'))->with('success', 'Leave Type updated successfully.');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'something went wrong.  ' . $e->getMessage())->withInput();
        }
    }
    ...
    public function destroy($id)
    {
        $hID = decrypt($id);
        try {
            $holiday = LeavePolicy::findOrFail($hID);
            $holiday->delete();
            return redirect(route('admin.leavePolicy.index'))->with('success', 'Leave Policy deleted successfully');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'something went wrong.  ' . $e->getMessage())->withInput();
        }
    }
