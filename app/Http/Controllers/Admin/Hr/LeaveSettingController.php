...
class LeaveSettingController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $user = auth()->user();
            if ($user->is_admin && !isset($user->organization_id)) { // Assuming superadmin has no organization_id or is_admin=1
                $data = LeaveSetting::orderBy('created_at', 'desc')->get();
            } else {
                $data = LeaveSetting::where('organization_id', $user->organization_id)->orderBy('created_at', 'desc')->get();
            }
            ...
            return DataTables::of($data)->addIndexColumn()
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href="' . route('admin.hr.leave-settings.edit', encrypt($row->id)) . '" class="btn btn-sm"><i class="fa fa-edit text-success"></i></a>';
                    $btn .= ' | ';
                    $btn .= '<form method="POST" action="' . route('admin.hr.leave-settings.destroy', encrypt($row->id)) . '" class="m-0 p-0">
        ...
        return view('admin.hr.leave_settings.index');
    }
    ...
    public function create()
    {
        ...
        return view('admin.hr.leave_settings.create', compact('department', 'designation'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->allotment_type === 'yearly') {
            ...
            if ($request->monthly_leave > 4) {
                return redirect()->back()->with('error', 'Monthly leave cannot be more than 4')->withInput();
            }
        }
        ...
        try {
            ...
            return redirect(route('admin.hr.leave-settings.index'))->with('success', 'Leave Type created successfully.');
        } catch (Exception $e) {
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            return view('admin.hr.leave_settings.edit', compact('lSetting', 'department', 'designation'));
        } catch (Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect(route('admin.hr.leave-settings.index'))->with('success', 'Leave Type updated successfully.');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'Something went wrong: ' . $e->getMessage())->withInput();
        }
    }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            $holiday->delete();
            return redirect(route('admin.hr.leave-settings.index'))->with('success', 'Leave Setting deleted successfully');
        } catch (Exception $e) {
            return redirect()->back()->with('error', 'Something went wrong: ' . $e->getMessage())->withInput();
        }
    }
