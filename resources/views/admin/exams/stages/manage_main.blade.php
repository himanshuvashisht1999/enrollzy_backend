...
@section('title', 'Manage Main Stage - ' . $exam->name)

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0 fw-bold">Manage Main Stage</h4>
    ...
    <form action="{{ route('admin.exams.main.update', $exam->id) }}" method="POST">
        @csrf
        ...
        <div class="card border-0 shadow-sm">
                ...
                <div class="tab-content" id="stageTabsContent">
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="stage_name" class="form-control"
                                    value="{{ old('stage_name', $main->stage_name ?? 'Main Exam') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Mandatory?</label>
                                <select name="mandatory" class="form-select">
                                    <option value="1" {{ old('mandatory', $main->mandatory ?? true) ? 'selected' : '' }}>Yes
                                    </option>
                                    <option value="0" {{ !old('mandatory', $main->mandatory ?? true) ? 'selected' : '' }}>No
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="subjects_required_raw" class="form-control"
                                    value="{{ old('subjects_required_raw', isset($main->subjects_required) ? implode(',', (array) $main->subjects_required) : '') }}">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Attempt Limit</label>
                                <input type="number" name="attempt_limit" class="form-control"
                                    value="{{ old('attempt_limit', $main->attempt_limit ?? '') }}">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Gap Year Allowed?</label>
                                <select name="gap_year_allowed" class="form-select">
                                    <option value="1" {{ old('gap_year_allowed', $main->gap_year_allowed ?? true) ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ !old('gap_year_allowed', $main->gap_year_allowed ?? true) ? 'selected' : '' }}>No</option>
                                </select>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label">Eligibility Notes</label>
                                <textarea name="eligibility_notes" class="form-control editor"
                                    rows="3">{{ old('eligibility_notes', $main->eligibility_notes ?? '') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <select name="exam_mode" class="form-select">
                                    <option value="Online" {{ old('exam_mode', $main->exam_mode ?? '') == 'Online' ? 'selected' : '' }}>Online (CBT)</option>
                                    <option value="Offline" {{ old('exam_mode', $main->exam_mode ?? '') == 'Offline' ? 'selected' : '' }}>Offline (Pen & Paper)</option>
                                    <option value="Hybrid" {{ old('exam_mode', $main->exam_mode ?? '') == 'Hybrid' ? 'selected' : '' }}>Hybrid</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Format</label>
                                <select name="exam_format" class="form-select">
                                    <option value="MCQ" {{ old('exam_format', $main->exam_format ?? '') == 'MCQ' ? 'selected' : '' }}>MCQ</option>
                                    <option value="Descriptive" {{ old('exam_format', $main->exam_format ?? '') == 'Descriptive' ? 'selected' : '' }}>Descriptive</option>
                                    <option value="Mixed" {{ old('exam_format', $main->exam_format ?? '') == 'Mixed' ? 'selected' : '' }}>Mixed</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">Total Qs</label>
                                <input type="number" name="total_questions" class="form-control"
                                    value="{{ old('total_questions', $main->total_questions ?? '') }}">
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">Total Marks</label>
                                <input type="number" name="total_marks" class="form-control"
                                    value="{{ old('total_marks', $main->total_marks ?? '') }}">
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">Duration (Mins)</label>
                                <input type="number" name="duration_minutes" class="form-control"
                                    value="{{ old('duration_minutes', $main->duration_minutes ?? '') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Negative Marking?</label>
                                <select name="negative_marking" class="form-select">
                                    <option value="1" {{ old('negative_marking', $main->negative_marking ?? false) ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ !old('negative_marking', $main->negative_marking ?? false) ? 'selected' : '' }}>No</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Marking Scheme</label>
                                <input type="text" name="negative_marking_scheme" class="form-control"
                                    value="{{ old('negative_marking_scheme', $main->negative_marking_scheme ?? '') }}"
                                    placeholder="e.g. -0.25">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Syllabus URL</label>
                                <input type="url" name="syllabus_url" class="form-control"
                                    value="{{ old('syllabus_url', $main->syllabus_url ?? '') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Difficulty</label>
                                <select name="difficulty_level" class="form-select">
                                    @foreach(['Easy', 'Moderate', 'Hard', 'Very Hard'] as $lvl)
                                        <option value="{{ $lvl }}" {{ old('difficulty_level', $main->difficulty_level ?? '') == $lvl ? 'selected' : '' }}>{{ $lvl }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-9">
                                ...
                                <input type="text" name="subjects_covered_raw" class="form-control"
                                    value="{{ old('subjects_covered_raw', isset($main->subjects_covered) ? implode(',', (array) $main->subjects_covered) : '') }}">
                            </div>
                            <div class="col-md-12">
                                <label class="form-label">Syllabus Source/Notes</label>
                                <textarea name="syllabus_source" class="form-control editor"
                                    rows="2">{{ old('syllabus_source', $main->syllabus_source ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="dates" role="tabpanel">
                        ...
                        <div id="sessions-container">
                            @php $sessions = old('sessions_data', $main->sessions_data ?? []); @endphp
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <textarea name="admit_card_download_procedure" class="form-control editor"
                                    rows="2">{{ old('admit_card_download_procedure', $main->admit_card_download_procedure ?? '') }}</textarea>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Result Check Procedure</label>
                                <textarea name="result_check_procedure" class="form-control editor"
                                    rows="2">{{ old('result_check_procedure', $main->result_check_procedure ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-3">
                                ...
                                <select name="score_type" class="form-select">
                                    <option value="Marks" {{ old('score_type', $main->score_type ?? '') == 'Marks' ? 'selected' : '' }}>Marks</option>
                                    <option value="Percentile" {{ old('score_type', $main->score_type ?? '') == 'Percentile' ? 'selected' : '' }}>Percentile</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Normalization Applied?</label>
                                <select name="normalization_applied" class="form-select">
                                    <option value="1" {{ old('normalization_applied', $main->normalization_applied ?? false) ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ !old('normalization_applied', $main->normalization_applied ?? false) ? 'selected' : '' }}>No</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Tie Breaking Rules</label>
                                <input type="text" name="tie_breaking_rules" class="form-control"
                                    value="{{ old('tie_breaking_rules', $main->tie_breaking_rules ?? '') }}">
                            </div>
                        </div>

                        <h6 class="fw-bold text-primary mb-3">Cutoff Information</h6>
                        <div id="cutoff-container">
                            @php $cutoffs = old('cutoff_year_wise', $main->cutoff_year_wise ?? []); @endphp
                    ...
                    <div class="tab-pane fade" id="fees" role="tabpanel">
                            ...
                            <div class="col-md-6">
                                <div class="form-check form-switch p-2 border rounded">
                                    <input class="form-check-input ms-0 me-2" type="checkbox"
                                        name="registration_fee_required" value="1" {{ old('registration_fee_required', $main->registration_fee_required ?? false) ? 'checked' : '' }}>
                        ...
                        <div id="fee-section-wrapper"
                            style="{{ old('registration_fee_required', $main->registration_fee_required ?? false) ? '' : 'display:none;' }}">
                            <div id="fee-container">
                                @php $feeStructure = old('registration_fee_structure', $main->registration_fee_structure ?? []); @endphp
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="payment_modes_allowed_raw" class="form-control"
                                    value="{{ old('payment_modes_allowed_raw', isset($main->payment_modes_allowed) ? implode(',', (array) $main->payment_modes_allowed) : '') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Tx Charges Applicable?</label>
                                <select name="transaction_charges_applicable" class="form-select">
                                    <option value="1" {{ old('transaction_charges_applicable', $main->transaction_charges_applicable ?? false) ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ !old('transaction_charges_applicable', $main->transaction_charges_applicable ?? false) ? 'selected' : '' }}>No</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Borne By</label>
                                <select name="transaction_charge_borne_by" class="form-select">
                                    <option value="Candidate" {{ old('transaction_charge_borne_by', $main->transaction_charge_borne_by ?? '') == 'Candidate' ? 'selected' : '' }}>
                                        Candidate</option>
                                    <option value="Authority" {{ old('transaction_charge_borne_by', $main->transaction_charge_borne_by ?? '') == 'Authority' ? 'selected' : '' }}>
                                        Authority</option>
                                </select>
                            </div>
                        </div>
            ...
            <div class="card-footer bg-white text-end py-3 border-top">
                <button type="submit" class="btn btn-primary px-5 shadow-sm">
                    <i class="fas fa-save me-2"></i> Save Main Stage Details
                </button>
            </div>
        </div>
    </form>
