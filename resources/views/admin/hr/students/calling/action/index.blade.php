...
<script>
    $(document).ready(function() {
        let table = $('#actionTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.hr.students.calling-actions.index') }}",
            columns: [
        ...
        $('#actionForm').on('submit', function(e) {
            ...
            let url = id ? "{{ url('admin/hr/students/calling-actions') }}/" + id : "{{ route('admin.hr.students.calling-actions.store') }}";
        ...
        $(document).on('click', '.edit-action', function() {
            let id = $(this).data('id');
            $.get("{{ url('admin/hr/students/calling-actions') }}/" + id, function(res) {
                if(res.status == 1) {
