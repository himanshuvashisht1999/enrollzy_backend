...
<script>
    $(document).ready(function() {
        let table = $('#statusTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.hr.students.calling-statuses.index') }}",
            columns: [
        ...
        $('#statusForm').on('submit', function(e) {
            ...
            let url = id ? "{{ url('admin/hr/students/calling-statuses') }}/" + id : "{{ route('admin.hr.students.calling-statuses.store') }}";
        ...
        $(document).on('click', '.edit-status', function() {
            let id = $(this).data('id');
            $.get("{{ url('admin/hr/students/calling-statuses') }}/" + id, function(res) {
                if(res.status == 1) {
