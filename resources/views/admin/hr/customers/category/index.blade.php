...
<script>
    $(document).ready(function() {
        let table = $('#categoryTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.hr.customer-categories.index') }}",
            columns: [
        ...
        $('#categoryForm').on('submit', function(e) {
            ...
            let url = id ? "{{ url('admin/hr/customer-categories') }}/" + id : "{{ route('admin.hr.customer-categories.store') }}";
        ...
        $(document).on('click', '.edit-category', function() {
            let id = $(this).data('id');
            $.get("{{ url('admin/hr/customer-categories') }}/" + id, function(res) {
                if(res.status == 1) {
