...
@section('title', 'Edit Customer Field')

@section('content')
<div class="container-fluid">
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-header bg-white py-3">
            <h6 class="m-0 fw-bold text-primary">Edit Configuration: {{ $field->label }}</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.hr.customer-fields.update', encrypt($field->id)) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label small fw-bold">Database Name (No spaces) <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control rounded-3" value="{{ $field->name }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-bold">Display Label <span class="text-danger">*</span></label>
                        <input type="text" name="label" class="form-control rounded-3" value="{{ $field->label }}" required>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label small fw-bold">Is Mandatory?</label>
                        <select name="is_required" class="form-select rounded-3">
                            <option value="0" {{ $field->is_required == 0 ? 'selected' : '' }}>Optional</option>
                            <option value="1" {{ $field->is_required == 1 ? 'selected' : '' }}>Required</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label small fw-bold">Display Sequence</label>
                        <input type="number" name="sequence" class="form-control rounded-3" value="{{ $field->sequence }}">
                    </div>
                    <div class="col-md-4">
                ...
                <div class="text-end mt-4">
                    <a href="{{ route('admin.hr.customer-fields.index') }}" class="btn btn-light rounded-pill px-4 me-2">Cancel</a>
