...
<div class="container-fluid">
    <div class="card shadow-sm border-0 rounded-4">
        <form id="editLeavePolicyForm" method="POST" action="{{ route('admin.hr.leave-policies.update', encrypt($lSetting->id)) }}">
            @csrf
            @method('PATCH')
            <div class="card-header bg-white py-3">
                <h6 class="m-0 fw-bold text-primary">Edit Leave Policy Details</h6>
            </div>
            <div class="card-body">
                @php
                    $selDeparts = explode(',', $lSetting->department_ids);
                    $selDesigns = explode(',', $lSetting->designation_ids);
                    $selStaffs  = explode(',', $lSetting->staff_ids);
                @endphp
                <div class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="policy" id="policyEditor" class="form-control">{{ old('policy', $lSetting->policy) }}</textarea>
                    </div>
                </div>
            </div>
            <div class="card-footer bg-white text-end py-3 border-0">
                ...
                <button type="submit" class="btn btn-primary rounded-pill px-4">Update Policy</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        // Similar AJAX logic as Create can be added here if needed for dynamic updates during edit
    });
</script>
@endpush
