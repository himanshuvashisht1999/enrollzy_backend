...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Add New Campus - {{ $organisation->organisation_name }}</h4>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <form action="{{ route('admin.organisations.campuses.store', $organisation->id) }}" id="campus-form"
                method="POST">
                @csrf
                    ...
                    <div class="card-header bg-white border-bottom-0 pb-0">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <div>
                                <span class="badge bg-soft-primary text-primary px-3 py-2 rounded-pill">
                                    <i class="fas fa-university me-1"></i> Campus Setup
                                </span>
                            </div>
                        ...
                        <ul class="nav nav-tabs-custom" id="campusTabs" role="tablist">
                            ...
                            <li class="nav-item"><a class="nav-link" href="#safety"
                                    role="tab">Safety & Security</a></li>
                            <li class="nav-item"><a class="nav-link" href="#contact"
                                    role="tab">Contact</a></li>
                            <li class="nav-item"><a class="nav-link" href="#class-profile"
                                    role="tab">Class Profile</a></li>
                        </ul>
                    </div>
                        ...
                        <div class="tab-content text-muted p-2">
                                        ...
                                        <select name="campus_type_new_id[]" class="form-select select2" multiple data-placeholder="Select School Type(s)">
                                            @if(isset($schoolTypes))
                                                @foreach($schoolTypes as $st)
                                                    <option value="{{ $st->id }}" {{ in_array($st->id, old('campus_type_new_id', [])) ? 'selected' : '' }}>
                                                        {{ $st->title }}
                                                    </option>
                                                @endforeach
                                            @endif
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select name="campus_area_unit" class="form-select">
                                                ...
                                                <option value="Square Yard">Sq Yard</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Classrooms Count</label>
                                        <input type="number" name="classrooms_count" class="form-control" value="0">
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" name="smart_classrooms"
                                                value="1" id="smart_class">
                                    ...
                                    @foreach($facilitiesMaster as $facility)
                                        <div class="col-md-3 col-sm-4 col-6">
                                            <div class="facility-card text-center border rounded p-3 h-100 position-relative" style="cursor: pointer;" onclick="toggleFacility('{{ $facility->id }}')">
                                                <input type="checkbox" name="facilities[]" value="{{ $facility->id }}" id="facility_{{ $facility->id }}" class="d-none" {{ in_array($facility->id, old('facilities', [])) ? 'checked' : '' }}>
                                    ...
                                    <div class="col-12" id="bus-routes-box" style="display:none;">
                                        ...
                                        <div id="bus-routes-container">
                                            <div class="input-group mb-2">
                                                ...
                                                <button type="button"
                                                    class="btn btn-outline-danger remove-route">Remove</button>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" name="fire_safety_certified"
                                                value="1" id="fire">
                                            <label class="form-check-label" for="fire">Fire Safety Certified</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="tab-pane fade" id="class-profile" role="tabpanel">
                                ...
                                <div id="class-profile-container">
                                    @php $classProfiles = old('class_profile', [[]]); @endphp
    ...
    <script>
        $(document).ready(function () {
            ...
            function saveStepData() {
                ...
                if (!campusId && currentTabId === 'identity') {
                    // Create Draft
                    const formData = {
                        campus_name: $('input[name="campus_name"]').val(),
                        campus_type: $('select[name="campus_type"]').val(),
                        _token: '{{ csrf_token() }}'
                    };

                    if (!formData.campus_name) return;

                    isSaving = true;
                    showAutoSaveStatus('saving');
                    $.ajax({
                        url: '{{ route("admin.organisations.campuses.store-draft", $organisation->id) }}',
                        method: 'POST',
                        data: formData,
                        success: function (response) {
                            ...
                            // Transform form to UPDATE mode to prevent duplicate on final submit
                            const updateUrl = `/admin/organisations/{{ $organisation->id }}/campuses/${campusId}`;
                            $('#campus-form').attr('action', updateUrl);
                            if ($('#campus-form input[name="_method"]').length === 0) {
                                $('#campus-form').append('<input type="hidden" name="_method" value="PUT">');
                            }
                        },
                        complete: function () {
                            isSaving = false;
            ...
            $(document).on('click', '.remove-route', function () { $(this).parent().remove(); });
            ...
            let classProfileIndex = {{ count(old('class_profile', [[]])) }};
